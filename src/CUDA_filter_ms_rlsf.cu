...
__device__
float CUDA_compute_weight_ms_rlsf(int* in_data, int width, float r, float g, float b, int pos, int alpha, float sigma, float* central_pix) {
...
__global__
void CUDA_denoise_pixel_rlsf(int* in_data, int* out_data, const int width, const int height, const int radius, const int alpha, const float sigma, const int iter)
{
	int f = 1;
	...
	do {
		...
		for (int i = istart; i <= iend; i++) { // i = y
			for (int j = jstart; j <= jend; j++) { // j = x
				int q = i * width + j;
				w = CUDA_compute_weight_ms_rlsf(in_data, width, 
...
CUDA_filter_ms_rlsf ( const Image * in_img, const int r, int alpha, const float sigma, const int iter)
{
 ...
 CUDA_denoise_pixel_rlsf << < gridDim, blockDim >> > (d_in_data, d_int_out_data, num_cols, num_rows, r, alpha, 2 * sigma * sigma, iter);
